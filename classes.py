...
class MSDFacilityLocationGaussian(MSDObjective):
    def __init__(self, passenger_coords, grid_coords, lambda_param, k, distortion, sensitivity):
        """
        Version: Exponential Metric (1 - e^-d)
        d = Manhattan distance
        """
        # Call the parent ABC constructor to set the sensitivity attribute
        self.passengers = passenger_coords
        self.grid = grid_coords
        self.lambda_param = lambda_param
        self.k = k
    ...
    def _get_similarity_row(self, grid_idx):
        """
        Calculates similarity using exponential decay: s(i, j) = e^(-d(i, j)).
        d is raw Manhattan distance.
        """
        # Raw L1 Distance
        diffs = np.abs(self.passengers - self.grid[grid_idx])
        l1_dists = np.sum(diffs, axis=1)

        # Exponential similarity (maps [0, inf) to (0, 1])
        return np.exp(-200*l1_dists)

    def evaluate(self, S):
        if not S:
            return 0.0, (np.zeros(self.num_passengers), 0.0)

        # 1. Coverage (Facility Location) term using e^-d
        max_sims = np.zeros(self.num_passengers)
        for idx in S:
            max_sims = np.maximum(max_sims, self._get_similarity_row(idx))
        ...
        # 2. Diversity (Max-Sum) term using 1 - e^-d
        dist_sum = 0.0
        n_S = len(S)
        if n_S > 1:
            for i in range(n_S):
                for j in range(i + 1, n_S):
                    # Distance in this metric is 1 - e^(-Manhattan)
                    raw_l1 = np.sum(np.abs(self.grid[S[i]] - self.grid[S[j]]))
                    dist_sum += (1.0 - np.exp(-raw_l1))

        avg_dist = (dist_sum / self.num_pairs) if self.num_pairs > 0 else 0.0

        # Weighted combination
        total_val = (1 - self.lambda_param) * self.distortion * coverage_term + \
                    (self.lambda_param * avg_dist)
    ...
    def marginal_gain(self, e, S, auxiliary):
        ...
        # 2. Diversity Gain (1 - e^-d)
        dist_to_existing = 0.0
        if S:
            # Vectorized Manhattan from candidate e to all hubs in S
            l1_to_hubs = np.sum(np.abs(self.grid[S] - self.grid[e]), axis=1)
            # Transform to metric: 1 - e^-d
            dist_to_existing = np.sum(1.0 - np.exp(-l1_to_hubs))

        new_dist_sum = current_dist_sum + dist_to_existing
        diversity_gain = (new_dist_sum - current_dist_sum) / self.num_pairs

        # Total gain calculation
        total_gain = (1 - self.lambda_param) * self.distortion * coverage_gain + \
                     (self.lambda_param * diversity_gain)
